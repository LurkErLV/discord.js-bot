...
module.exports = {
  ...
  execute(message, args) {
    if (message.member.hasPermission('ADMINISTRATOR')) {
      ...
      if (target === undefined) {
        message.channel.send(`<@${message.author.id}>, !setstat @<пользователь> exp/lvl/money/admin значение`);
        return;
      }
      if (stats[1] === 'lvl') {
        db.run(`UPDATE users SET lvl = ? WHERE userid = ?`, [stats[2], target.id]),
        function(err) {
          console.log(err);
        };
        ...
        db.run(`UPDATE users SET xp = ? WHERE userid = ?`, [stats[2], target.id]),
        function(err) {
          console.log(err);
        };
        ...
        db.run(`UPDATE users SET money = ? WHERE userid = ?`, [stats[2], target.id]),
        function(err) {
          console.log(err);
        };
        ...
        if (stats[2] === 'true' || stats[2] === 'false') {
          db.run(`UPDATE users SET admin = ? WHERE userid = ?`, [stats[2], target.id]),
          function(err) {
            console.log(err);
          };
          ...
          message.channel.send(`Неизвестный параметр '${stats[2]}', доступные параметры 'true' и 'false'!`);
        }
      } else {
